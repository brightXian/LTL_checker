...
def main():
    ...
    global_formulas, local_formulas = parse_benchmark(benchmark_path)

    def run(check_ts, formula_str):
        formula = parse(formula_str)
        gnba = ltl_to_gnba(Not(formula))
    ...
    for formula_str in global_formulas:
        print(1 if run(ts, formula_str) else 0)
    ...
    for state_index, formula_str in local_formulas:
        ...
        print(1 if run(local_ts, formula_str) else 0)
